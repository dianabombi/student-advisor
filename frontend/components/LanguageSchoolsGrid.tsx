...
export default function LanguageSchoolsGrid() {
    ...
    useEffect(() => {
        async function fetchSchools() {
            try {
                const response = await fetch(`/api/universities?jurisdiction_code=${jurisdiction}&type=language_school`);
                const data = await response.json();
                setSchools(data.universities || []);
            } catch (error) {
                console.error('Failed to fetch language schools:', error);
            } finally {
                setLoading(false);
            }
        }
    ...
    const handleChatOpen = (school: LanguageSchool) => {
                    ...
                    >
                        ...
                        <div className="p-4">
                            ...
                            <p className="text-gray-700 mb-3 line-clamp-2">
                                {t(`languageSchools.${school.id}.description`)}
                            </p>
                            ...
                            <button
                                className="w-full bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-lg font-semibold transition-colors duration-300 flex items-center justify-center gap-2"
                                onClick={(e) => {
                                    e.stopPropagation();
