#!/usr/bin/env python3
"""
Scrape universities from remaining countries: HU, SI, HR, GR, LI, NL
"""
import sys
import os
...
def main():
    ...
    try:
        ...
        jurisdictions = db.query(Jurisdiction).filter(
            Jurisdiction.code.in_(['HU', 'SI', 'HR', 'GR', 'LI', 'NL'])
        ).all()
        ...
        print(f"🇭🇺🇸🇮🇭🇷🇬🇷🇱🇮🇳🇱 Found {len(universities)} universities in HU, SI, HR, GR, LI, NL")
        ...
        print(f"\n✅ Queued {len(universities)} remaining universities!")
