#!/usr/bin/env python3
"""
Scrape universities from French-speaking countries: FR, BE, LU, MC
"""
import sys
import os
...
def main():
    ...
    try:
        ...
        # Get French-speaking jurisdictions
        jurisdictions = db.query(Jurisdiction).filter(
            Jurisdiction.code.in_(['FR', 'BE', 'LU', 'MC'])
        ).all()
        ...
        print(f"🇫🇷🇧🇪🇱🇺🇲🇨 Found {len(universities)} universities in FR, BE, LU, MC")
        ...
        print(f"\n✅ Queued {len(universities)} French-speaking universities!")
