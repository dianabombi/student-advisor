...
# -*- coding: utf-8 -*-
"""
Повний тест визначення міста на ВСІХ 10 мовах платформи
"""
...
def test_all_languages():
    """Тестування визначення міста на всіх 10 мовах"""
    service = JobsChatService()
    
    # Тести для всіх 10 мов: sk, cs, pl, en, de, fr, es, uk, it, ru
    test_cases = [
        # ========== KOŠICE ==========
        # Словацька (sk)
    ...
    print("=" * 100)
    print("ПОВНИЙ ТЕСТ ВИЗНАЧЕННЯ МІСТА НА ВСІХ 10 МОВАХ ПЛАТФОРМИ")
    print("Мови: sk, cs, pl, en, de, fr, es, uk, it, ru")
    print("=" * 100)
    ...
    for lang in ['sk', 'cs', 'pl', 'en', 'de', 'fr', 'es', 'uk', 'it', 'ru']:
        if lang in results_by_language:
            stats = results_by_language[lang]
            total = stats['passed'] + stats['failed']
            success_rate = stats['passed'] / total * 100 if total > 0 else 0
            status = "✅" if stats['failed'] == 0 else "⚠️"
            print(f"{status} {language_names[lang]:12} ({lang}): {stats['passed']}/{total} ({success_rate:.0f}%)")
        else:
            print(f"⚪ {language_names[lang]:12} ({lang}): не тестувалась")
