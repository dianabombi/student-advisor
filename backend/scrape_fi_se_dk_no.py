#!/usr/bin/env python3
"""
Scrape universities from Scandinavian countries: FI, SE, DK, NO
"""
import sys
import os
...
def main():
    ...
    try:
        ...
        # Get Scandinavian jurisdictions
        jurisdictions = db.query(Jurisdiction).filter(
            Jurisdiction.code.in_(['FI', 'SE', 'DK', 'NO'])
        ).all()
        ...
        print(f"🇫🇮🇸🇪🇩🇰🇳🇴 Found {len(universities)} universities in FI, SE, DK, NO")
        ...
        print(f"\n✅ Queued {len(universities)} Scandinavian universities!")
