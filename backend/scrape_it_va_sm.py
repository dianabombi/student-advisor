#!/usr/bin/env python3
"""
Scrape universities from Italian-speaking countries: IT, VA, SM
"""
import sys
import os
...
def main():
    ...
    try:
        ...
        # Get Italian-speaking jurisdictions
        jurisdictions = db.query(Jurisdiction).filter(
            Jurisdiction.code.in_(['IT', 'VA', 'SM'])
        ).all()
        ...
        print(f"🇮🇹🇻🇦🇸🇲 Found {len(universities)} universities in IT, VA, SM")
        ...
        print(f"\n✅ Queued {len(universities)} Italian-speaking universities!")
